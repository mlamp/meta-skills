...
Run every command below only while `HEAD` equals the commit that last changed `freeze.json`, after that commit has landed on the default branch. Complete qualification, adapter smoke, interviews, tasks, judgments, and blind human adjudication first.
