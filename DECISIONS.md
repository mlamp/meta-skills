...
## D-033 · 2026-08-25 · The publication sanitizer matches real secrets, not ordinary task prose (amends D-032)

The pack-time sanitizer scans every raw member, and a measured task record carries the subject model's answer verbatim: `call_record` sanitizes the call metadata but stores `result` unchanged. Four frozen E-09 patterns matched the vocabulary the frozen tasks ask for. `bearer_secret` matched the phrase "Bearer token", which T03 invites by asking the model to review a design that logs an Authorization token. `host_home_path` matched `/home/user/...` and `/Users/you/...`, which T01 invites by asking for a restore path. `host_temp_path` matched `/tmp/config.yaml`, which T02 invites. `credential_assignment` matched `AUTH_TOKEN=<redacted>`, the redaction a correct T03 answer recommends. There is no waiver, so one hit destroys a whole batch at packaging time, after every call is spent.

Those four patterns now require a value shaped like a real secret or a real host path, and exempt the placeholder and prose forms the frozen tasks invite. `experiments/e09/artifact-spec.json` at the frozen commit owns the exact expressions; this entry does not restate them, because they are tuned against a corpus and the file is their only source of truth. `host_windows_path`, `host_uuid`, and `host_project_id` are unchanged. `host_project_id` already catches the harness's own `e09-tool-`, `e09-text-`, and `e09-codex-` working directories wherever they appear.

Weakening a leak control needs a pinned safety argument, so the corpus is a test, not a claim in prose. `test_sanitizer_blocks_real_leaks_without_blocking_task_prose` in `experiments/e09/test_harness.py` loads the spec and asserts both directions: every leak string matches, and every benign string matches nothing. It covers real credential formats, this machine's own repository and temporary paths, and the task answers that previously blocked. CI runs that suite on every pull request, so a later edit that reopens either failure fails the build.

The pattern set is a two-frame detector, not a credential scanner. A real key in free prose, with no `Bearer` prefix and no `NAME=` frame, still matches nothing; so do non-Bearer schemes, a quoted or backticked token, and name shapes like `AWS_SECRET_ACCESS_KEY=`. Those gaps predate this entry. The defence that does cover a configured credential in any shape is the exact-value scan, which compares stored bytes against the credential values loaded from the ignored `.env` file and the environment. The narrowing is safe for E-09 because only `result` is unsanitized, and it is written by subject models running in safe mode with no tools, no filesystem, and no credential, and by a judge whose output schema admits only booleans and a fixed task id. A spec reused where the subject has tools or credentials needs a wider scanner first.

`experiments/artifact-storage-smoke/artifact-spec.json` keeps the earlier expressions on purpose. Its published manifest pins that file's hash, so syncing it would break remote verification of an immutable release that CI re-checks.

Amending `artifact-spec.json` changed `freeze.json`, so E-09's measured id and both cold-reader qualification keys moved. No measured provider call had been spent, so nothing was discarded. The reader-smoke and adapter-smoke keys do not bind the freeze and were unaffected. A sanitizer pattern that can match a frozen task's own vocabulary is a design defect to fix before measurement, never a batch to retry.

## D-034 · 2026-08-26 · Binding text states its own obligations, and a one-shot gate is measured before it is spent (amends D-005)

E-09's cold-reader qualification is one attempt per key: 680 assertions across 50 case-reps, zero tolerance, and a burn costs a new freeze and a new measured id. It had never been run, so the first attempt would have been a blind bet. `experiments/e09/rehearse_qualification.py` runs the same cases through the same code path and writes nothing, so the odds can be measured before the attempt is consumed. Measure a one-shot gate before spending it.

The first rehearsal returned 22 of 50 case-reps clean. Every failure was deterministic, so repetition tolerance was never the remedy. Two of the four defects were in the catalog, which is also the E-09 treatment arm. It said `Overlap precedence: U01 > ... > U06.` and nothing more: never that a span can match several entries, never that the earliest wins even when a later entry describes it better. It also never said that an entry with no listed sublabels takes no sublabel, so `U01` with an `opener` sublabel looked legal. Two independent model families read past both, deterministically. Under D-005 that is binding text failing its cold reader, and the response is to fix the text, not to dismiss the reader. Binding text states its own obligations; an ordering that is only implied is not stated.

The other two defects were in the cases. C1/P8 keyed "This design is better" as `abstraction` while the catalog defines `promotion` as "a promotional quality or superiority label used instead of evidence"; the span matched both sublabels and nothing tie-breaks sublabels the way `overlap_precedence` tie-breaks pattern ids. C4 never defined `rejected_ids`, and C4/S1 claimed to test an explicit rejection while its text stated a dislike. A case that does not test what it claims is a defect to repair; a case its readers simply find hard is not.

Repairing the cases and the catalog is legitimate. Iterating on either until the gate opens is not, because the catalog wording is what C19–C22 measure. The rule is: correct what contradicts the frozen definitions or what a cold reader demonstrably misreads, then re-measure and report. Stop there.

After the repair, C0, C1 and C2 returned zero failing assertions for both readers. C3 and C4 still fail, so the gate stays shut and no measured call has been spent. One repair is suspect: the C4/S1 rewrite improved haiku and regressed DeepSeek, which previously returned the keyed `selected_ids` every time and now often returns none. It is recorded on #4 as a candidate revert rather than silently re-edited, because deciding between two readings of that item is a design judgement, not a defect correction.

The sublabel rule is enforced by catalog wording, not by the submission schema. `validate_schema` implements no conditional keywords, and a constraint the harness enforced but a provider did not would surface as a format error, which gets no retry and fails the case outright — the same failure class the change exists to remove.
