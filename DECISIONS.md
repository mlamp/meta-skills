...
The release tag is unique and resolves to the exact frozen commit already contained in the default branch. The manifest pins the repository by numeric id and name, the repo-relative raw root, frozen schedule hashes, the frozen and packager commits, every frozen input hash, every archive member hash and kind, expected and actual counts, exclusions, errors, retries, the sanitization report, and any superseded release. Inventory counts include every planned record, including failures and exclusions; separate status counts preserve those outcomes. The packager admits only the exact expected set of regular JSON or JSONL files. It rejects links, unsafe paths, malformed data, unexpected or missing members, configured credential values, and frozen secret or host patterns. Any credential or pattern match blocks publication. There is no waiver; correct the frozen sanitizer or inventory and start a new batch. Sorted member order and normalized tar and gzip metadata make two packs of the same bytes identical.

The order is package, verify locally, stage the draft, inspect the draft, publish, verify remotely, then finalize. Expected and actual inventory must match, sanitization and digests must pass, and every error, retry, and exclusion must agree with the frozen protocol and raw records. An unexplained outcome or sensitive value blocks publication. `publish-release` publishes, waits for immutability, performs the first fresh-download and attestation verification, then copies the released manifest bytes into the tracked path. `finalize` independently repeats the fresh-download, tag, member, and attestation checks against that tracked manifest before writing compact results or ledger evidence. CI repeats the same verification with read-only permissions. A bad published bundle is never replaced: a new frozen batch names the old release in `supersedes`. Unpublished or abandoned work is not evidence and does not use `supersedes`.
