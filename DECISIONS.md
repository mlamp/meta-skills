...
Here, `freeze_sha256` is the SHA-256 of the frozen `freeze.json` bytes. A compact result is a schema-2 object. Its `lines` array equals every and only ledger row whose `results_path` names that file.
