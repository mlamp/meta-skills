...
- NTPD Non Third Person Description — frontmatter description in first or second person ("I help you…", "Use me when…"). Doesn't count: imperative mood in the body — this smell is about the description field only.
...
- MT Missing Template — output must follow a format but no template or skeleton is given. Doesn't count: genuinely free-form outputs.

### Grounding & examples

- ME Missing Example — an example would change behavior, and there is none — or only a placeholder ("<YOUR_CODE_HERE>", lorem ipsum), which counts as missing (C09). Doesn't count: sections where an example adds nothing.
