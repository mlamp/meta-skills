...
Copilot's seventh PR pass found two inline and one suppressed gap. Reader and adapter smoke now create and validate both the keyed base and new attempt namespaces before a provider call. The adapter-smoke key now includes the cold-reader case-suite hash. Before qualification or measured input validation, the harness matches the working `freeze.json` and every frozen input byte to the exact frozen commit through link-safe repository paths. Regressions pin the case-suite key, source mismatch, gate ordering, and both smoke namespace checks.

Fable 5 xhigh and Kimi K3 high independently reviewed the focused repair. Both returned `CLEAN` on the security behavior. Fable noted that the first regression stopped at the base namespace check; it now also proves that an invalid attempt namespace blocks the provider. Its unrelated 1000-attempt naming observation cannot admit stale evidence and was not added to this repair. Kimi found one stale design sentence that omitted the case-suite hash; the old sentence was corrected.

A fresh Claude Haiku 4.5 process with no project settings, tools, or session context read only the final binding-text diff. It correctly restated both smoke namespace checks, all four adapter-key inputs, the exact working-byte gates and their ordering, restart behavior, provider identity, remote-source and symlink rejection, and the external-root rule. It reported `CONTRADICTIONS: NONE`.

The artifact suite has 52 tests, the E-09 harness suite has 101, and the agent-voice artifact suite has 24. Reviewer advice, cold reads, and these tests are context, not effectiveness evidence. No measured E-09 provider call ran.
